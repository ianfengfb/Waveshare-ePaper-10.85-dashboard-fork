...
# how many real tasks come back, so both this widget's height and the
# filler carousel corner are fixed, predictable blocks rather than growing
...
def draw_spotify_weather_corner(draw, Himage, rect, fonts, spotify, weather, aqi):
    ...
    MIDDLE_COLUMN_WIDGET selects, ported from the retired 3-column layout's
    widget (deleted once every corner had real content ported in — see
    CLAUDE.md) at a fraction of its old footprint. No overlay logic needed
    here any more (see CLAUDE.md): the hydration nag
...
def draw_news_calendar_corner(draw, Himage, rect, fonts, news_headlines, nasa_apod, calendar_events):
    ...
    ported essentially unchanged from the retired 3-column layout's own
    filler block (deleted once every corner had real content ported in —
    see CLAUDE.md). Greeting and Email moved out to the clock corner, so
    nothing new joins this rotation — it's the same
    filler_slides/dots/fallback logic at a smaller size, not a redesign.
...
def render_screen(epd, fonts):
    ...
    if clock_showing:
        ...
        # 3-column layout's own greeting widget (deleted once every corner
        # had real content ported in — see CLAUDE.md): a time-of-day word
